...
int main() {
	...
	std::cerr<<"rot(0)  A="<<sfm::Angle(rotated)<<std::endl;
	sfm::Rotate(rotated, PI/2.0);
	std::cerr<<"rot(90) Vx="<<rotated.x<<std::endl;
	std::cerr<<"rot(90) Vy="<<rotated.y<<std::endl;
	std::cerr<<"rot(90)  A="<<sfm::Angle(rotated)<<std::endl;
	sfm::Rotate(rotated, PI/2.0);
	std::cerr<<"rot(180) Vx="<<rotated.x<<std::endl;
	std::cerr<<"rot(180) Vy="<<rotated.y<<std::endl;
	std::cerr<<"rot(180)  A="<<sfm::Angle(rotated)<<std::endl;
	sfm::Rotate(rotated, PI/2.0);
	std::cerr<<"rot(270) Vx="<<rotated.x<<std::endl;
	std::cerr<<"rot(270) Vy="<<rotated.y<<std::endl;
	std::cerr<<"rot(270)  A="<<sfm::Angle(rotated)<<std::endl;
	sfm::Rotate(rotated, -PI*1.50);
	std::cerr<<"rot(-270) Vx="<<rotated.x<<std::endl;
	std::cerr<<"rot(-270) Vy="<<rotated.y<<std::endl;
	std::cerr<<"rot(-270)  A="<<sfm::Angle(rotated)<<std::endl;

	float angle=sfm::Angle(rotated);

	float radius=sfm::Length(vector2);
	std::cerr<<"R="<<radius<<std::endl;

	sf::Vector2f norm=vector2;
	sfm::Normalize(norm);
	radius=sfm::Length(norm);
	std::cerr<<"R="<<radius<<std::endl;
	return EXIT_SUCCESS;
}
