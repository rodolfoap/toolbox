...
int main(int args, char *argv[]) { // setup our window and a few resources we need
	slWindow(400, 400, "Simple SIGIL Example", 0);
	//slSetFont(slLoadFont("ttf/arial.ttf"), 24);
	slSetFont(slLoadFont("arial.ttf"), 24);
	slSetTextAlign(SL_ALIGN_CENTER);
	int tex = slLoadTexture("glow.png");

	while (!slShouldClose() && !slGetKey(SL_KEY_ESCAPE)) { // background glow
