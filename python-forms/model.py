...
class MainForm(nps.Form):
	...
	def __init__(self, name, data):
		print('DATA', data)
		self.fieldList=data
		for k,v in self.fieldList.items():
			print(k,':',v)
	...
	def create(self):
		...
		for k,v in self.fieldList.items():
			print(k,':',v)
			self.field[k]=self.add(v, name=k, value=v)
...
class Servc(nps.NPSAppManaged):
	def onStart(self):
		a={'Name': nps.TitleText, 'Filename': nps.TitleFilename}
		f=MainForm(name='New Employee', data=a)
